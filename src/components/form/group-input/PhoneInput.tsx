...
const PhoneInput: React.FC<PhoneInputProps> = ({
  ...
  const handleCountryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setPhone(countryCodes[newCountry]);
    if (onChange) {
  ...
  const handlePhoneNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const maQG = newPhoneNumber.trim().split(" ")[0] + " ";
    const found = countries.find(c => c.label === maQG);
    const newCT = String(found?.code);
    setSelectedCountry(newCT);
    if (onChange) {
